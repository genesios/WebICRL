  ...
  public class CotizacionICRL
  {
    ...
    public static bool DaniosPropiosSumatoriaGenerar(int Flujo, int Cotizacion, short Tipo_Item)
    {
      ...
      string strComando = "INSERT INTO [dbo].[cotizacion_danios_propios_sumatoria] " +
        ...
        "SELECT [id_flujo],[id_cotizacion],[id_tipo_item],[proveedor],SUM(precio_final),'Fijo',0.0,0.0,0.0,'',0,[id_moneda] FROM [dbo].[cotizacion_danios_propios] " +
    ...
    public class TipoOrden
    {
      ...
      public string descripcion;
      public double monto_bs;
      public double monto_us;
      public short id_estado;
      public TipoOrden()
      {
        this.id_flujo = 0;
    ...
    public static bool OrdenRegistrar(TipoOrden Orden)
    {
      bool blnRespuesta = false;
      string strComando = "INSERT INTO [dbo].[cotizacion_ordenes] ([id_flujo],[id_cotizacion],[tipo_origen],[numero_orden],[fecha_orden],[descripcion],[monto_bs],[monto_us],[id_estado]) " +
        "VALUES (@id_flujo,@id_cotizacion,@tipo_origen,@numero_orden,@fecha_orden,@descripcion,@monto_bs,@monto_us,@id_estado) ";
      SqlConnection sqlConexion = new SqlConnection(strCadenaConexion);
      SqlCommand sqlComando = new SqlCommand(strComando, sqlConexion);
      try
      {
        ...
        sqlComando.Parameters.Add("@monto_us", System.Data.SqlDbType.Float).Value = Orden.monto_us;
        sqlComando.Parameters.Add("@id_estado", System.Data.SqlDbType.SmallInt).Value = Orden.id_estado;
        sqlConexion.Open();
        sqlComando.ExecuteNonQuery();
        blnRespuesta = true;
      }
    ...
    public class TipoOrdenTraer
    {
      public bool Correcto;
      public string Mensaje;
      public List<TipoOrden> Ordenes = new List<TipoOrden>();
      public System.Data.DataSet dsOrdenes = new System.Data.DataSet();
    }
    public static TipoOrdenTraer OrdenTraer(int Flujo, int Cotizacion)
    {
      TipoOrdenTraer objRespuesta = new TipoOrdenTraer();
      SqlConnection sqlConexion = new SqlConnection(strCadenaConexion);
      string strComando = "SELECT [id_item],[tipo_origen],[numero_orden],[fecha_orden],[descripcion],[monto_bs],[monto_us],[id_estado] FROM [dbo].[cotizacion_ordenes] WHERE [id_flujo]=@id_flujo AND [id_cotizacion] =@id_cotizacion";
      SqlCommand sqlComando = new SqlCommand(strComando, sqlConexion);
      SqlDataAdapter sqlAdaptador = new SqlDataAdapter(strComando, sqlConexion);
      SqlDataReader sqlDatos;
      TipoOrden tdpFila;
      try
      {
        ...
        while (sqlDatos.Read())
        {
          ...
          if (sqlDatos["descripcion"] != DBNull.Value) tdpFila.descripcion = Convert.ToString(sqlDatos["descripcion"]);
          if (sqlDatos["monto_bs"] != DBNull.Value) tdpFila.monto_bs = Convert.ToDouble(sqlDatos["monto_bs"]);
          if (sqlDatos["monto_us"] != DBNull.Value) tdpFila.monto_us = Convert.ToDouble(sqlDatos["monto_us"]);
          if (sqlDatos["id_estado"] != DBNull.Value) tdpFila.id_estado = Convert.ToInt16(sqlDatos["id_estado"]);

          objRespuesta.Ordenes.Add(tdpFila);
        }
        sqlDatos.Close();
        sqlAdaptador.SelectCommand.Parameters.Add("@id_flujo", System.Data.SqlDbType.Int).Value = Flujo;
        sqlAdaptador.SelectCommand.Parameters.Add("@id_cotizacion", System.Data.SqlDbType.Int).Value = Cotizacion;
        sqlAdaptador.Fill(objRespuesta.dsOrdenes);
        objRespuesta.Correcto = true;
      }
      catch (Exception ex)
      {
    ...
    public static bool OrdenModificar(TipoOrden Orden)
    {
      bool blnRespuesta = false;
      string strComando = "UPDATE [dbo].[cotizacion_ordenes] SET [tipo_origen] = @tipo_origen,[numero_orden] = @numero_orden,[fecha_orden] = @fecha_orden,[descripcion] = @descripcion,[monto_bs] = @monto_bs,[monto_us] = @monto_us,[id_estado] = @id_estado " +
        "WHERE [id_flujo] = @id_flujo AND [id_cotizacion] = @id_cotizacion AND [id_item] = @id_item";
      SqlConnection sqlConexion = new SqlConnection(strCadenaConexion);
      SqlCommand sqlComando = new SqlCommand(strComando, sqlConexion);
      try
