  ...
  public partial class CotizacionRCVehicular : System.Web.UI.Page
  {
    ...
    protected void ButtonRepaGrabar_Click(object sender, EventArgs e)
    {
      ...
      if ("A" == TextBoxRepaFlagEd.Text)
      {
        ...
        if (("OTRO (S)" == vTipoRCVehicular.item_descripcion) && (!string.IsNullOrWhiteSpace(TextBoxRepaItem.Text)))
        {
          vTipoRCVehicular.item_descripcion = TextBoxRepaItem.Text.ToUpper();
        }
      }
      else
      {
    ...
    protected void ButtonRepuGrabar_Click(object sender, EventArgs e)
    {
      ...
      if ("A" == TextBoxRepuFlagEd.Text)
      {
        ...
        if (("OTRO (S)" == vTipoRCVehicular.item_descripcion) && (!string.IsNullOrWhiteSpace(TextBoxRepuItem.Text)))
        {
          vTipoRCVehicular.item_descripcion = TextBoxRepuItem.Text.ToUpper();
        }
      }
      else
      {
    ...
    protected void PImprimeFormularioCotiRCVehicular(string pNroOrden)
    {
      ...
      var vListaCotiRCVehicularTercero = from c in db.cotizacion_rc_vehicular
                                         join f in db.Flujo on c.id_flujo equals f.idFlujo
                                         join t in db.cotizacion_rc_vehicular_tercero on c.id_cotizacion equals t.id_cotizacion
                                         where (c.numero_orden == pNroOrden)
                                         select new
                                         {
                                           f.flujoOnBase,
    ...
    protected void ButtonTallerGenCambiar_Click(object sender, EventArgs e)
    {
      ...
      if (string.Empty != TextBoxTallerGen.Text)
      {
        ...
        if (vIndiceDataTable >= 0)
        {
          ...
          BD.CotizacionICRL.RCVehicularSumatoriaModificarTodos(vDatasetOrdenes, vNombreTallerGen);
